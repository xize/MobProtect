...
public class denyPlayer {
	
	public static void setDenyPlayer(CommandSender sender, String[] args) {
						...
						try {
							...
							if(f.exists()) {
								...
								if(users.contains(args[1])) {
									...
									sender.sendMessage(ChatColor.GREEN + "you have successfully removed " + args[1] + " from your region allow list");
									con.save(f);
								} else {
									sender.sendMessage(ChatColor.RED + args[1] + " is allready removed!");	
								}
							} else {
								sender.sendMessage(ChatColor.RED + args[1] + " is allready removed!");
							}
